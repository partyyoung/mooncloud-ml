...
public class ParallelFeatureSelect implements Tool
{
	...
	static class FSMapper extends Mapper<LongWritable, Text, IntIntPair, Text>
	{
				...
				throws IOException, InterruptedException
		{
			...
			if (line.length == m + 1)
			{
				...
				for (int i = 0; i < featureLeftStringArray.length; i++)
				{
					...
					context.write(
							new IntIntPair(Integer
									.parseInt(featureLeftStringArray[i]), sb
									.toString().hashCode()), new Text(line[m]));
				...
				InterruptedException
		{
			...
			for (int i = 0; i < featureLeftStringArray.length; i++)
				featureLeftIntArray[i] = Integer
						.parseInt(featureLeftStringArray[i]);
			...
			if (!featureSelect.equals(" ") && !featureSelect.equals(""))
			{
				...
				for (int i = 0; i < featureSelectStringArray.length; i++)
					featureSelectIntArray[i] = Integer
							.parseInt(featureSelectStringArray[i]);
			}

		}
	}

	static class FSMapper1 extends
			Mapper<LongWritable, Text, IntIntPair, TextLongPair>
	{
			...
			if (line.length == m + 1)
			{
				...
				for (int i = 0; i < featureLeftStringArray.length; i++)
				{
					...
					context.write(
							new IntIntPair(Integer
									.parseInt(featureLeftStringArray[i]), sb
									.toString().hashCode()), new TextLongPair(
									line[m], new Long(1)));
	...
	static class FSCombiner extends
			Reducer<IntIntPair, TextLongPair, IntIntPair, TextLongPair>
	{
		public void reduce(IntIntPair key, Iterable<TextLongPair> values,
	...
	static class FSConsistencyReducer1 extends
			Reducer<IntIntPair, TextLongPair, IntWritable, LongWritable>
	{
	...
	static class FSConsistencyReducer extends
			Reducer<IntIntPair, Text, IntWritable, LongWritable>
	{
		...
		public void reduce(IntIntPair key, Iterable<Text> values,
				Context context) // 注意：这里要用Iterable，不能用Iterator，不知道为什么。
				throws IOException, InterruptedException
		{
	...
	static class FSDependencyReducer extends
			Reducer<IntTextPair, Text, IntWritable, LongWritable>
	{
	...
	static class SIGMapper extends
			Mapper<LongWritable, Text, IntWritable, LongWritable>
	{
				...
				throws IOException, InterruptedException
		{
			...
			context.write(new IntWritable(Integer.parseInt(line[0])),
					new LongWritable(Long.parseLong(line[1])));
		}
	}

	static class SIGReducer extends
	...
	static class FeaturePartitioner extends
			Partitioner<IntIntPair, TextLongPair>
	{
		public int getPartition(IntIntPair key, TextLongPair value,
				int numPartitions)
		{
	...
	static class FeatureGroupComparator implements RawComparator<IntTextPair>
	{
		...
		public int compare(byte[] arg0, int arg1, int arg2, byte[] arg3,
				int arg4, int arg5)
		{
			return WritableComparator.compareBytes(arg0, arg1,
					Integer.SIZE / 8, arg3, arg4, Integer.SIZE / 8);
		}
	...
	private int getFeatureNumber(String inputpath, Configuration conf)
			throws IOException
	{
		int M = 0;
	...
	public int run(String[] args) throws Exception
	{
		...
		{
			System.err
					.println("Usage: ParallelFeatureSelect <taskTrackers> <input path> <output tmp path> <output path>");
			System.exit(-1);
		}
		...
		conf.set("fs.default.name",
				"file:///home/yangjd/Documents/workspace/ParallelFeatureSelect/");
		...
		while (iteration)
		{
			...
			job1.setJarByClass(ParallelFeatureSelect.class);
			...
			job2.setJarByClass(ParallelFeatureSelect.class);
...
//			fstm.delete(outDir, true);
			...
			} else
			{
				...
				featureLeft = (" " + featureLeft + " ").replaceFirst(
						" " + fs + " ", " ").trim();
	...
	public static void main(String[] args) throws Exception
	{
		...
		int exitCode = ToolRunner.run(new ParallelFeatureSelect(), args);
